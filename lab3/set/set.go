...
func Intersection(sets ...Interface) Interface {
	...
	// 	}
		...
		for _, set := range sets {
			// fmt.Println(item)
			// it's old threads. They were only able to make paralel calculation for each set, but the problem that all elements are 
			// compareing not in threads. So look into realizetion of Each method
			wg.Add(1)
